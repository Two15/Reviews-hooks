    ...
    if (body.match(regexp)) {
      ...
      updatedReviews.forEach(function(reviewer) {
        if ([':-1:', '👎'].indexOf(newStatus.trim()) >= 0) {
          reviewers[reviewer.toLowerCase()].state = 'failure';
          reviewers[reviewer.toLowerCase()].description = `@${author} rejected for @${reviewer}`;
        }
        if ([':+1:', '👍'].indexOf(newStatus.trim()) >= 0) {
          reviewers[reviewer.toLowerCase()].state = 'success';
          reviewers[reviewer.toLowerCase()].description = `@${author} accepted for @${reviewer}`;
        }
      });
    }
    return reviewers;
